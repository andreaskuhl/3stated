...
function wConfig.existWidgetKey(functionName, key)
    ...
    wHelper.Debug:new(widget.no, functionName):error("Widget key '" .. key .. "' not found!")
    return false
end
...
function wConfig.startPanel(title, i)
    local panelTitle = STR(CFL(title))
    if i then panelTitle = panelTitle .. " " .. i end
    panel = form.addExpansionPanel(panelTitle)
    panel:open(false)
end
...
------------------------------------------------------------------------------------------------------------------------
--- Add line
function wConfig.addLineTitle(title)
    local line
    if panel then
...
function wConfig.addLine(key, i)
    ...
    return wConfig.addLineTitle(title)
end
